...
async def generate_and_store_embeddings_for_note(note: models.Note, db: AsyncSession):
    ...
    # 5. Generate embeddings for all chunks via Google Embeddings API
    embeddings = await embed_texts(MODEL_NAME, text_chunks)
    # Filter out any empty embeddings; align chunks accordingly
    filtered_pairs = [(c, e) for c, e in zip(db_chunks, embeddings) if isinstance(e, list) and len(e) > 0]
    if not filtered_pairs:
        # No embeddings produced; skip indexing but keep chunks in DB
        await db.commit()
        return
    db_chunks, embeddings = zip(*filtered_pairs)
    ...
    await crud.create_embeddings_for_chunks(db, db_chunks=list(db_chunks), embeddings=embeddings_np.tolist(), model_name=MODEL_NAME)
